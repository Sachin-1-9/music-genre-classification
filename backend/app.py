from flask import Flask, request, jsonify
from flask_cors import CORS
import os
import pickle
import numpy as np
import pandas as pd
import librosa
import tempfile
import subprocess

app = Flask(__name__)
CORS(app)

# Allow large uploads (300 MB)
app.config["MAX_CONTENT_LENGTH"] = 300 * 1024 * 1024

# --- Absolute paths (works no matter where you run from) ---
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR = os.path.abspath(os.path.join(BASE_DIR, ".."))

MODEL_PATH = os.path.join(ROOT_DIR, "model", "genre_classifier.pkl")
FEATURES_CSV_PATH = os.path.join(ROOT_DIR, "features.csv")

# --- Supported file types ---
AUDIO_EXTENSIONS = [".wav", ".mp3", ".ogg", ".flac", ".m4a", ".au", ".aiff", ".aif"]
VIDEO_EXTENSIONS = [".mp4", ".mov", ".mkv", ".avi", ".webm"]

# --- Load model ---
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(f"Model not found at: {MODEL_PATH}")

with open(MODEL_PATH, "rb") as f:
    model = pickle.load(f)

# --- Read feature schema (order matters) ---
if not os.path.exists(FEATURES_CSV_PATH):
    raise FileNotFoundError(f"features.csv not found at: {FEATURES_CSV_PATH}")

_expected_cols = list(pd.read_csv(FEATURES_CSV_PATH, nrows=0).columns)
if "label" in _expected_cols:
    _expected_cols.remove("label")


def ffmpeg_extract_wav(video_path: str, out_wav_path: str):
    """
    Extract mono WAV audio at 22050 Hz from a video using ffmpeg.
    Works reliably on macOS if ffmpeg is installed.
    """
    cmd = [
        "ffmpeg",
        "-y",                      # overwrite
        "-i", video_path,          # input
        "-vn",                     # no video
        "-ac", "1",                # mono
        "-ar", "22050",            # sample rate
        out_wav_path
    ]
    # Capture output so errors are readable
    p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if p.returncode != 0:
        raise RuntimeError(f"ffmpeg failed: {p.stderr[-800:]}")  # last part of log


def extract_features_matching_schema(audio_path: str) -> np.ndarray:
    """
    Match your Day-2 extraction: 55 features total:
      - MFCC mean (40)
      - Chroma mean (12)
      - Spectral centroid mean (1)
      - Spectral rolloff mean (1)
      - ZCR mean (1)
    """
    y, sr = librosa.load(audio_path, duration=30, mono=True)

    mfcc_mean = np.mean(librosa.feature.mfcc(y=y, sr=sr, n_mfcc=40).T, axis=0)      # 40
    chroma_mean = np.mean(librosa.feature.chroma_stft(y=y, sr=sr).T, axis=0)        # 12
    spec_centroid = float(np.mean(librosa.feature.spectral_centroid(y=y, sr=sr)))   # 1
    spec_rolloff = float(np.mean(librosa.feature.spectral_rolloff(y=y, sr=sr)))     # 1
    zcr = float(np.mean(librosa.feature.zero_crossing_rate(y)))                     # 1

    raw = np.hstack([mfcc_mean, chroma_mean, spec_centroid, spec_rolloff, zcr]).astype(np.float32)

    # Align to CSV schema length (expected: 55)
    out = np.zeros(len(_expected_cols), dtype=np.float32)
    m = min(len(out), len(raw))
    out[:m] = raw[:m]
    return out.reshape(1, -1)


@app.get("/")
def home():
    return jsonify({
        "message": "Music Genre Classification API running",
        "expected_features": len(_expected_cols),
        "use": "POST /predict with form-data key 'file' (audio or video)"
    })


@app.post("/predict")
def predict():
    # ✅ Public endpoint – no token required
    if "file" not in request.files:
        return jsonify({"error": "No file uploaded (form-data key must be 'file')"}), 400

    f = request.files["file"]
    filename = (f.filename or "uploaded").strip()
    lower = filename.lower()
    _, ext = os.path.splitext(lower)

    if ext not in AUDIO_EXTENSIONS + VIDEO_EXTENSIONS:
        return jsonify({"error": f"Unsupported file type: {ext}"}), 400

    temp_input = None
    temp_wav = None

    try:
        # Save upload to temp file
        with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as tmp:
            temp_input = tmp.name
            f.save(temp_input)

        # Convert video to wav if needed
        source = "audio"
        audio_path = temp_input

        if ext in VIDEO_EXTENSIONS:
            source = "video"
            with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmpw:
                temp_wav = tmpw.name
            ffmpeg_extract_wav(temp_input, temp_wav)
            audio_path = temp_wav

        # Extract features + predict
        X = extract_features_matching_schema(audio_path)
        genre = model.predict(X)[0]

        return jsonify({
            "genre": str(genre),
            "source": source,
            "features_used": int(X.shape[1]),
            "filename": filename
        })

    except Exception as e:
        return jsonify({"error": str(e)}), 500

    finally:
        for p in [temp_input, temp_wav]:
            try:
                if p and os.path.exists(p):
                    os.remove(p)
            except Exception:
                pass


if __name__ == "__main__":
    # Keep 5000 to match your UI API_URL
    app.run(host="127.0.0.1", port=5000, debug=False)
